...
app.post('/admin-login', (req, res) => {
    ...
    if (username === 'admin' && password === 'admin123') {
        ...
        pool.query("SELECT last_login FROM admins WHERE username = ?", [username], (err, results) => {
            if (err) return res.status(500).json({ error: "Database error" });

            const lastLogin = results.length > 0 ? results[0].last_login : null;
            pool.query("UPDATE admins SET last_login = ? WHERE username = ?", [currentTime, username], (err) => {
                if (err) return res.status(500).json({ error: "Update error" });

                pool.query("SELECT category, description, status, sname, timestamp FROM complaints WHERE timestamp > ? AND status = 'uncleared'", [lastLogin || '1970-01-01'], (err, complaints) => {
...
app.post('/handleform', upload.single('filepath'), (req, res) => {
    ...
    pool.query("INSERT INTO complaints (category, description, filepath, sname, email, status, timestamp) VALUES (?, ?, ?, ?, ?, 'uncleared', ?)", 
...
app.post('/suggestform', (req, res) => {
    ...
    pool.query("INSERT INTO suggest (name, suggestcol) VALUES (?, ?)", [name, suggestcol], (err) => {
        if (err) return res.status(500).send("Registration unsuccessful");
...
app.get('/fetch-complaints', (req, res) => {
    pool.query("SELECT category, description, status, filepath FROM complaints", (err, results) => {
...
app.put('/update-status', (req, res) => {
    ...
    pool.query("UPDATE complaints SET status = ? WHERE category = ? AND description = ?", [status, category, description], (err, result) => {
        if (err) return res.status(500).send("Failed to update status");
        if (status === 'cleared') {
            pool.query("SELECT sname, email FROM complaints WHERE category = ? AND description = ?", [category, description], (err, results) => {
                if (err) return res.status(500).send("Error fetching email");
